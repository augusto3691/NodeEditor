(function (jsPlumbInstance) {
    jsPlumbInstance.load = function (options, plumbInstance)
    {
        ...
        for (var k = 0; k < endPoints.length; k++) {
            plumbInstance.addEndpoint(
                    endPoints[k].source,
                    {anchor: [endPoints[k].pos]},
            EndpointOptions
                    );
        }
...
//            console.log(connections[i].endpoint);
            ...
            var connection1 = plumbInstance.connect({
                ...
                connector: [connections[i].connectorType, {stub: [40, 40], gap: 10, cornerRadius: 10, alwaysRespectStubs: true}],
                labelStyle: {
    ...
    jsPlumbInstance.save = function (options, plumbInstance)
    {
        ...
        $(options.selector).each(function (idx, elem) {
            ...
            var ep = plumbInstance.getEndpoints($elem.attr('id'));
            for (var index in ep) {
                endPoints.push({
                    source: ep[index].elementId,
                    pos: ep[index].anchor.anchors[0].type
                });
            }
                ...
                anchors: function ()
                {
                    ...
                    connection[i].endpoints.forEach(function (endpoint) {
                        ...
                        var tempObj = {
                            ...
                            x: endpoint.anchor.x,
                            y: endpoint.anchor.y,
                            orientation: ((endpoint.anchor.anchors[0].type == "Right") ? [1, 0] : [-1, 0]),
        ...
        var obj = {selector: options.selector, connections: connections, blocks: blocks, endPoints: endPoints};
        return obj;
    };
})(jsPlumb);